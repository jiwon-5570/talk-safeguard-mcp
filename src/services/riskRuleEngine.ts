...
import { generateSafetyGuide } from "./safetyGuideService.js";
...
function classifyRules(message: string): MatchedRule[] {
  ...
  const hasUrl = /(?:https?|hxxps?):\/\//iu.test(message);
  ...
  if (family.length > 0 && (money.length > 0 || avoidance.length > 0)) {
...
export function classifyMessage(message: string): ScamClassification {
  return processEphemeral(message, (ephemeralMessage) => {
    ...
    return {
      ...
      evidence,
      explanation: `${primary.type} 유형과 관련된 복합 위험 신호 ${evidence.length}개가 감지되었습니다. 이는 가능성 분류이며 확정 판정이 아닙니다.`,
    };
  });
}
...
export function analyzeMessage(message: string, userSituation: UserSituation = "unknown"): MessageRiskAnalysis {
  return processEphemeral(message, (ephemeralMessage) => {
