...
export function analyzeMessageRiskTool(input: unknown) {
  const { message, userSituation, receivedVia, userQuestion } = AnalyzeMessageInputSchema.parse(input);
  const result = analyzeMessage(message, userSituation, receivedVia, userQuestion);
